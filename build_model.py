...
outside_dat =  pd.read_csv('../Data/outside_datmay16.csv', 
...
drop_cols = ['_projectid', '_teacher_acctid', '_schoolid', 'school_ncesid', 'school_city', 'school_state', 'school_zip', 
             ...
             'days_to_completion', 'vendor_shipping_charges', 'city_state_cat', 'city_state_counts', 'school_metro', 'keep_drop', 'year_week']
...
WiseGamma.set_num_threads(WiseGamma.get_num_cores()+2)

train = DataSet.load('Documents/wise/Data/training_set.csv')
test = DataSet.load('Documents/wise/Data/testing_set.csv')

# Look at data to make sure that it loaded properly
print "training set # observations: " + str(train.num_instances)
print "training set # variables: " + str(train.num_variables)
print "testing set # observations: " + str(test.num_instances)
print "testing set # variables: " + str(test.num_variables)

# Model training and prediction - OOB method 

model1 = Model.learn(train, 'funded_by_30', feature_importance_method="oob", num_trees = 150, 
                     splitting_criterion = 'infogain', categorical_split_optimizer = 'hybrid') 
# Using: 150 trees, 4 features to sample per split

predictions1 = model1.probs(test)

# Create vectors of predicted probabilities and observed values for comparison
probs1 = predictions1["Yes"]
actual = test.to_pandas()['funded_by_30']

# Check AUC
fpr, tpr, thresholds = metrics.roc_curve(actual, probs1, pos_label = 'Yes')
roc_auc = auc(fpr, tpr)
print "Area under the ROC curve : %f" % roc_auc

responses = model1.predict(test)

probs1.to_csv('Documents/wise/Data/probs1.csv', index = False)
actual.to_csv('Documents/wise/Data/actual.csv', index = False)
responses.to_csv('Documents/wise/Data/actual.csv', index = False)

# # ---------------------------------------------- # # 
# #     STEP 4 & 5: SAVE MODEL       # #
# # ---------------------------------------------- # #

model1.save('Documents/wise/Data/model1')
